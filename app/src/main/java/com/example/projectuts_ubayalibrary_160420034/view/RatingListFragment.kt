...
import com.example.projectuts_ubayalibrary_160420034.viewmodel.RatingListViewModel

class RatingListFragment : Fragment() {
    private lateinit var viewModel: RatingListViewModel
    private var ratingListAdapter = RatingListAdapter(arrayListOf())
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val recView =view.findViewById<RecyclerView>(R.id.recViewRating)
    ...
    fun observeViewModels(){
        ...
        viewModel.loadingLD.observe(viewLifecycleOwner, Observer {
            val progressLoad = view?.findViewById<ProgressBar>(R.id.progressBarRating)
            val recView = view?.findViewById<RecyclerView>(R.id.recViewRating)
            if(it == true){
                recView?.visibility = View.GONE
